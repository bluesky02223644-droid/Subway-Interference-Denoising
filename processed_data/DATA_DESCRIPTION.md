...
| `figure5_BJI_denoised_2024-07-13.npz` | Figure 5(a)(c)(e) | Beijing (BJI) station, 2024-07-13, 24-hour Z-component: `original` and `denoised` arrays (86,400 samples at 1 Hz) |
...
| `first_order_diff_1384days.csv` | Section 3.3 (first-order difference analysis) | Per-day statistics for 1,384 station-days (BJI 609d + HZC 186d + TAY 589d), computed with the unified evaluation pipeline: columns `station`, `date`, `den_std` (standard deviation of the denoised first-order differences), `den_max` (maximum absolute first-order difference), `den_p95` (95th percentile). Taking the per-station median of each column reproduces the manuscript first-order difference table. |
| `tableS4_TAY_589days_metrics.csv` | Table S4, Section 3.3 | Per-day released-model metrics for all 589 withheld Taiyuan days: columns `date`, `E_band`, `SNR_dB`, `H_band`, `bandRMS`, `ESR`, `CCpsd`, `SIF`, `fdP95` (95th percentile of the absolute first difference of the denoised signal). Grouping the days by `SNR_dB` (below the 25th, between, and above the 75th percentile) and taking the per-group median of each column reproduces Table S4. |
